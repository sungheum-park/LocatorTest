...
<script type="text/javascript">
    ...
    function jsList(page) {
        ...
        $.ajax({
            url: "<%=ColorController.COLOR_LIST%>",
            data: data,
    ...
    function jsRegForm() {
        location.href="<%=ColorController.COLOR_REG_FORM%>";
    }

    function jsModForm(no_color) {

        var form = document.createElement("form");
        var parm = new Array();
        var input = new Array();

        form.action = "<%=ColorController.COLOR_MOD_FORM%>";
        form.method = "post";


        parm.push( ['no_color', no_color] );
    ...
    function jsDel() {
        ...
        var del_length = $("#listForm").find("[name=del_no_color]:checked").length;

        if(del_length<1) {
            alert("삭제할 상품 색상을 선택해 주세요.");
            return;
        }
        ...
        $.ajax({
            url: "<%=ColorController.COLOR_DEL_ACTION%>",
            data: data,
            contentType: "application/x-www-form-urlencoded; charset=UTF-8",
            type: "POST",
    ...
    function jsExcelDown(){
        ...
        f.action = '<%=ColorController.COLOR_LIST_EXCEL_DOWNLOAD%>';
        f.submit();
    }
...
<div class="contents">
    <div class="contents_now">
        <span>항목</span>
        <span>상품 관리</span>
        <span class="now_page">상품 색상 리스트</span>
    </div>
    ...
    <div class="common_title_wrap">
        ...
        <div class="btn_wrap">
            <div class="btn"><a href="javascript:;" onclick="jsRegForm();">+ 상품 색상 등록</a></div>
        </div>
    </div>
            ...
            <form name="searchForm" id="searchForm" onsubmit="return false;" method="post">
                ...
                <table>
                    ...
                    <tr class="search_word">
                        ...
                        <td class="infotd">RGB 색상</td>
                        <td>
                            <div class="td_inner">
                                <div class="input_wrap w230">
                                    <input type="text" name="color_rgb">
                                </div>
                            </div>
                        </td>
                    </tr>
