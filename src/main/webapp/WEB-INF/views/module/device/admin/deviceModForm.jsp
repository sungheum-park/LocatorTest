...
<script type="text/javascript">
    ...
    function jsModAction(){
        ...
        $("#modForm").ajaxForm({
            url: "<%=DeviceController.DEVICE_MOD_ACTION%>",
            type: "POST",
            data: data,
    ...
    function jsSetDevice(){
        var val = $("#modForm").find("[name=device_rgb]").val();
        if(val.length==7)
            $("#modForm .device_chip").css("background-device",val);
    }
...
<div class="side_menu_wrap">
    <ul>
        <li class="menu_title">
            ...
            <ul>
                <li class="on"><h4><a href="<%=DeviceController.DEVICE%>">상품 리스트</a></h4></li>
...
<div class="contents productRegister">
    <div class="contents_now">
        <span>항목</span>
        <span>상품 관리</span>
        <span class="now_page">상품 수정</span>
    </div>
    ...
    <!-- 공통 검색폼 -->
    <form id="modForm" name="modForm" onsubmit="return false;" method="post" enctype="multipart/form-data">
    <input type="hidden" name="no_device" value="${vo.no_device}">
                        ...
                        <td>
                            <div class="td_inner">
                                <div class="option_list w230">
                                    ${ufn:getSelectByTable("tb_category", "no_cate", "no_cate", "cate_name", "N", false, vo.no_cate)}
                                </div>
                            </div>
                        </td>
                    </tr>
                        ...
                        <td>
                            <div class="td_inner">
                                <div class="input_wrap w283 ml0">
                                    <input type="text" name="device_name" value="${vo.device_name}" maxlength="100">
                                </div>
                            </div>
                        </td>
                    </tr>
                            ...
                            <div class=image>
                                <div class="image_wrap">
                                    <div class="button_90">
                                        ...
                                        <input class="upload-name  upload_img" name="device_master_file_name" readonly="readonly" value="${vo.origin_name}">
                                        <div class="file_name">
                    ...
                    <tr class="period">
                        ...
                        <td class="inner_table">
                            <div class="td_inner">
                                ...
                                <span class="notice mt0">매장 상세 교환 현황에 들어가는 이미지입니다.</span>
                            </div>
                            <table class="color_grid">
                                ...
                                <tbody id="color_setting_area" class="sortable">
                                    <c:forEach var="list" items="${vo.map_array}" varStatus="status">
                                        <tr>
                                            <input type="hidden" name="selected_no_color" value="${list.no_color}">
                                            <input type="hidden" name="selected_color_sno" value="${list.color_sno}">
                                            <td class="sno_holder">${status.count}</td>
                                            <td class="color_name">${list.color_name}</td>
                                            <td>
                                                <div class="color">
                                                    <span>${list.color_rgb}</span>
                                                    <span class="color_chip" style="background-color:${list.color_rgb};  border: 1px solid #bfbfbf;"></span>
                                                </div>
                                            </td>
                                            <td class=image>
                                                <div class="image_wrap">
                                                    <div class="button_90">
                                                        <input type="file" id="file_${list.no_color}" name="device_file_${list.no_color}" class="download_btn blind upload-hidden" onchange="fileValidate(this);">
                                                        <label for="file_${list.no_color}" class="download_btn">찾아보기</label>
                                                        <input class="upload-name upload_img" name="device_file_name" readonly="readonly" value="${list.origin_name}">
                                                        <div class="file_name">
                                                            <span class="file_thumb" onclick="jsImagePreview(this)">
                                                                <img src="${list.file_path}${list.save_name}" alt="">
                                                            </span>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <td class="inner_table">
                                                <div class="option_list">
                                                    ${ufn:getSelectBox("limit_yn","Y","selected_limit_yn", false, list.limit_yn)}
                                                </div>
                                            </td>
                                        </tr>
                                    </c:forEach>
